...
public class TasksApplication {
	...
	CommandLineRunner runner(TaskService taskService){
		return args -> {
			...
			taskService.save( new Task(2L, "Create Spring Project Packages Application", LocalDate.now().plus(1, ChronoUnit.DAYS), false));
			taskService.save( new Task(3L, "Create the Task Domain Class", LocalDate.now().plus(3, ChronoUnit.DAYS), false));
			taskService.save( new Task(4L, "Create service and repository Class", LocalDate.now().plus(5, ChronoUnit.DAYS), false));
